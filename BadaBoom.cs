...
namespace Tanks
{
    class BadaBoom : Blow
    {
        ...
        public BadaBoom(int Direction, int Speed, ConsoleColor Color, COORD BoomPosition, int FoeIdentification) : base()
        {
        ...
        {
            {
                if (Position.x != 0 && Position.y != 0)
                {
                    ...
                    for (int i = -3; i <= 3; i++)
                    {
                        COORD Temp1 = new COORD(Position.x + i, Position.y - 3);
                        if (!FaceBorder(Temp1))
                        {
                            Coordinates[BoomRadius].Add(Temp1);
                        }
                        COORD Temp2 = new COORD(Position.x + i, Position.y + 3);
                        if (!FaceBorder(Temp2))
                        {
                            Coordinates[BoomRadius].Add(Temp2);
                        }
                    ...
                    for (int j = -2; j <= 2; j++)
                    {
                        COORD Temp1 = new COORD(Position.x - 3, Position.y + j);
                        if (!FaceBorder(Temp1))
                        {
                            Coordinates[BoomRadius].Add(Temp1);
                        }
                        COORD Temp2 = new COORD(Position.x + 3, Position.y + j);
                        if (!FaceBorder(Temp2))
                        {
                            Coordinates[BoomRadius].Add(Temp2);
                        }
